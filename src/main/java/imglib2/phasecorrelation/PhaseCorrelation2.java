...
public class PhaseCorrelation2 {
	...
	public static <T extends ComplexType<T>, S extends ComplexType<S>, R extends RealType<R>> RandomAccessibleInterval<R> calculatePCMInPlace(
			RandomAccessibleInterval<T> fft1, RandomAccessibleInterval<S> fft2, ImgFactory<R> factory, ExecutorService service){
	...
	public static <T extends RealType<T>, S extends RealType<S>, R extends RealType<R>, C extends ComplexType<C>> RandomAccessibleInterval<R> calculatePCM(
			...
			ImgFactory<R> factory, ImgFactory<C> fftFactory, ExecutorService service){
		...
		RandomAccessibleInterval<C> fft1 = fftFactory.create(fftSize);
		RandomAccessibleInterval<C> fft2 = fftFactory.create(fftSize);
		...
		RandomAccessibleInterval<R> pcm = calculatePCMInPlace(fft1, fft2, factory, service);
		return pcm;
	 ...
	 */
	public static <T extends RealType<T>, S extends RealType<S>, R extends RealType<R>, C extends ComplexType<C>> RandomAccessibleInterval<R> calculatePCM(
			RandomAccessibleInterval<T> img1, RandomAccessibleInterval<S> img2, ImgFactory<R> factory,
			ImgFactory<C> fftFactory, ExecutorService service) {
		
		int [] extension = new int[img1.numDimensions()];
		Arrays.fill(extension, 10);
		return calculatePCM(img1, img2, extension, factory, fftFactory, service);
	}
	...
	public static void main(String[] args) {
		...
		Pair< RandomAccessibleInterval<FloatType>, RandomAccessibleInterval<FloatType>> res = PhaseCorrelation2Util.dummyFuse(img1, img2, shiftPeak,service);
