...
import filter.MedianRealRandomAccessible;
...
public class Vistools
{
	public static void main( String[] args )
	{
		...
		final RealPointSampleList< FloatType > medianFiltered = Filters.filterMedian( data, outofbounds, medianRadius );
		...
		BdvFunctions.show( Render.renderGauss( data, outofbounds, gaussRenderRadius, gaussRenderSigma ), stdata.renderInterval, "Pcp4_rawgauss", BdvOptions.options().is2D() ).setDisplayRange( 0, 4 );
		BdvFunctions.show( Render.renderGauss( medianFiltered, outofbounds, gaussRenderRadius, gaussRenderSigma ), stdata.renderInterval, "Pcp4_rawgauss", BdvOptions.options().is2D() ).setDisplayRange( 0, 4 );

		// gauss smooth
		gaussRenderSigma = displayRadius;
		gaussRenderRadius = displayRadius * 4;

		BdvFunctions.show( Render.renderGauss( data, outofbounds, gaussRenderRadius, gaussRenderSigma ), stdata.renderInterval, "Pcp4_rawgauss", BdvOptions.options().is2D() ).setDisplayRange( 0, 9 );
		BdvFunctions.show( Render.renderGauss( medianFiltered, outofbounds, gaussRenderRadius, gaussRenderSigma ), stdata.renderInterval, "Pcp4_rawgauss", BdvOptions.options().is2D() ).setDisplayRange( 0, 9 );
		...
		//BdvFunctions.show( median, stdata.renderInterval, "Pcp4", BdvOptions.options().is2D() ).setDisplayRange( 0, 6 );
